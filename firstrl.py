...
class Object:
    ...
    def __init__(self, x, y, char, name, color, blocks=False, always_visible=False, fighter=None, ai=None, item=None, equipment=None):
        ...
        if self.equipment:  #let the Equipment component know who owns it
            ...
            #there must be an item component for the Equipment component to work correctly
            self.item = Item()
            self.item.owner = self
    def move(self, dx, dy):
    ...
    def draw(self):
        #only show if it's visible to the player or it has been explored and is "always visible"
        if (libtcod.map_is_in_fov(fov_map, self.x, self.y) or
            (self.always_visible and map[self.x][self.y].explored)):
            #set the color and then draw the character that represents this object at it's position
...
class Item:
    ...
    def use(self):
        #special case: if the object has the Equipment component, the "use" action is equip/dequip
        if self.owner.equipment:
...
class Equipment:
    #an object that can be equipped, yielding bonusus. Automatically add the Item component
    ...
    def dequip(self):
        #un-equip object and show a message about it
...
def make_map():
    ...
    for r in range(MAX_ROOMS):
        ...
        if not failed:
            #this means there are no intersesctions, so this room is valid
            ...
            room_no = Object(new_x, new_y + 1, chr(65+num_rooms), 'room number', libtcod.white,always_visible=True)
            ...
            if num_rooms == 0:
                ...
                if libtcod.random_get_int(0, 0, 1) == 1:
                    #first move horizontally, then virtically
                    create_h_tunnel(prev_x, new_x, prev_y)
                    create_v_tunnel(prev_y, new_y, new_x)
                else:
                    #first move vertically, then horizonatally
...
def random_choice(chances_dict):
    #choose one option from dictionary of choices, return its key
...
def place_objects(room):
    ...
    #maximum number of monsters per room [value, level]
    ...
    #item_chances['dagger'] = from_dungeon_level([[5, 1], [0, 4]])  #player currently starts with a dagger
...
def closest_monster(max_range):
    ...
    closest_enemy = None
    closest_dist = max_range + 1  #start with (slightly more tan) maximum range
